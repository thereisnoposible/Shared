...
//-------------------------------------------------------------------------------------------
void NetService::OnConnect(boost::shared_ptr<tcp::socket> psocket, boost::system::error_code ec)
{
	if (ec)
		return;
	boost::shared_ptr<NetConnect> conn(new NetConnect(psocket, boost::bind(&NetService::GetNetPack, this, _1)));
...
//-------------------------------------------------------------------------------------------
void NetService::OnDisConnect(boost::shared_ptr<NetConnect> pConnetct)
{
...
//-------------------------------------------------------------------------------------------
void NetService::GetNetPack(boost::shared_ptr<NetPack>& pPack)
{
	boost::mutex::scoped_lock lock(_packmutex);
...
//-------------------------------------------------------------------------------------------
bool NetService::Connect(const std::string& ip, int port, boost::function<void(boost::shared_ptr<NetConnect>&)> func)
{
	boost::shared_ptr<tcp::socket> skt(new tcp::socket(_io_service_pool.get_io_service()));
	...
	boost::shared_ptr<NetConnect> conn = boost::shared_ptr<NetConnect>(new NetConnect(skt, boost::bind(&NetService::GetNetPack, this, _1)));
