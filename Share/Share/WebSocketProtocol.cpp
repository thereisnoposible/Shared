...
//-------------------------------------------------------------------------------------
bool WebSocketProtocol::handshake(boost::shared_ptr<boost::asio::ip::tcp::socket>& pChannel, const char* s, int rpos)
{
	std::string data;
...
bool WebSocketProtocol::decodingDatas(const char* data,int len, NetPack* pPacket, unsigned char msg_masked, unsigned int msg_mask)
{
	...
	if (msg_masked)
	{
		...
		//pPacket->m_Head.datasize = ntohs(pPacket->m_Head.datasize);
