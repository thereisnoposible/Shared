#include "../include/timer/TimeWheel.h"
...
namespace xlib{
	...
	namespace timewheel
	{
		...
		Timer* TimeWheel::CreateTimer(long long diff, std::function<void()> cb_func, long long interval, int count)
		{
			int day = (int)(diff / 86400000);
			int hour = (int)((diff / 3600000) % 24);
			int min = (int)((diff / 60000) % 60);
			int sec = (int)((diff / 1000) % 60);
			int msec = (int)((diff % 1000) * msec_wheel.size() / 1000);
			...
			if (AddTime(hour, pTimer->hour, hour_point, 24,
				...
				AddTime(msec, pTimer->msec, msec_point, msec_wheel.size(), false)))))
				day++;
		...
		void TimeWheel::excuteTimer(Timer* pTimer)
		{
			...
			int day = (int)(pTimer->interval / 86400000);
			int hour = (int)((pTimer->interval / 3600000) % 24);
			int min = (int)((pTimer->interval / 60000) % 60);
			int sec = (int)((pTimer->interval / 1000) % 60);
			int msec = (int)((pTimer->interval % 1000) * msec_wheel.size() / 1000);

			if (AddTime(hour, pTimer->hour, pTimer->hour, 24,
				AddTime(min, pTimer->min, pTimer->min, 60,
				AddTime(sec, pTimer->sec, pTimer->sec, 60,
				AddTime(msec, pTimer->msec, pTimer->msec, msec_wheel.size(), false)))))
				day++;
			pTimer->day = day_point + day;
