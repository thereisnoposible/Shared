...
std::string MD5(const std::string& strPlain)
{
	MD5_CTX mdContext;

	MD5Init(&mdContext);
	MD5Update(&mdContext, (unsigned char*)const_cast<char*>(strPlain.c_str()), (int32)strPlain.size());
	MD5Final(&mdContext);
	...
	for (int i = 0; i < 16; i++)
	{
		sprintf_s(buf, 3, "%02x", mdContext.digest[i]);
		md5.append(buf);
	}
	return md5;
}
