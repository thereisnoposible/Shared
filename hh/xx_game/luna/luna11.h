...
{
    template<typename U> static auto check_gc(int) -> decltype(std::declval<U>()._gc(), std::true_type());
    template<typename U> static std::false_type check_gc(...);
    enum { value = std::is_same<decltype(check_gc<T>(0)), std::true_type>::value };
};

template <class T>
typename std::enable_if<has_member_gc<T>::value, void>::type lua_handle_gc(T* obj) { obj->_gc(); }
