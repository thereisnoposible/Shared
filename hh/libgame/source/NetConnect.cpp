...
{
	//close();
	if (xApplication::GetSingletonPtr() != NULL)
	{
		xApplication::GetSingleton().PrintMessage(msgDebug, "call NetConnect::~NetConnect,addr=%s", m_Address.c_str());
	}
}
...
void NetConnect::handleBody(const boost::system::error_code& error, size_t len)
{
	...
	{//收到完整的协议包，把包打入NetServer，等待update统一处理
		...
		{
			//LogService::GetSingleton().LogMessage("recv message, id=%d,role=%d,addr=%s,len=%d", m_recvBuff->GetMessageID(), m_recvBuff->GetRoleID(),m_Address.c_str(),m_recvBuff->GetDataSize());
...
void NetConnect::doSend()
{
	...
	if (BaseApp::GetSingletonPtr() != NULL)
	{
		//LogService::GetSingleton().LogMessage("send message,id=%d,role=%d,addr=%s,len=%d", m_sendPack->GetMessageID(), m_sendPack->GetRoleID(),m_Address.c_str(),m_sendPack->GetDataSize());
...
//---------------------------------------------------------------------------------------------
void NetConnect::releaseRes()
{
	boost::mutex::scoped_lock oLock(m_SendMutex);
	m_SendPackCollect.clear();
	oLock.unlock();

	m_Socket.get_io_service().post(boost::bind(&NetConnect::handle_close, shared_from_this()));
}
...
void NetConnect::handle_close()
{
	...
	catch (...)
	{
		...
		xApplication::GetSingleton().PrintMessage(msgDebug, "error when close socket");
	}
}