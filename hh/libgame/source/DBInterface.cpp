...
time_t MySQLQuery::getDateField(const char* szField,time_t t)
{
	...
	xstring sDate = getStringField(nField);
							...
							uint32 port ,const char* charset, unsigned long flag )
{
	...
	if (NULL == mysql_real_connect(_db_ptr, host, user, passwd, db, port, NULL, flag))
	{
		...
		LogService::GetSingleton().LogMessage("%s", error);
		return false;
	}
...
//--------------------------------------------------------------------
bool  DBInterface::querySQL(const xstring& sql, MySQLQuery& pQuery)
{
	...
	{
		const char* error=mysql_error(_db_ptr);
		LogService::GetSingleton().LogMessage("%s,%s",sql.c_str(),error);
...
//--------------------------------------------------------------------
int32 DBInterface::execSQL(const xstring& sql)
{
	if( !mysql_real_query( _db_ptr, sql.c_str(), sql.length() ) )
	{
		...
		if (effectrow == -1)
		{
			LogService::GetSingleton().LogMessage("%s", sql.c_str());
		}
		//得到受影响的行数
		return effectrow ;
	}
...
//--------------------------------------------------------------------
bool  DBInterface::execProcedurce(const xstring&  sql)
{
...
int32 DBInterface::startTransaction()
{
	...
	else
	{
		...
		LogService::GetSingleton().LogMessage("start transaction,%s",error);
...
int32 DBInterface::commit()
{
	...
	else
	{
		...
		LogService::GetSingleton().LogMessage("commit,%s",error);
