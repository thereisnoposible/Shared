...
: m_bQuit(false)
{
	m_pThread = new boost::thread(boost::bind(&HTTPClient::workThreadFun, this));
}
...
void HTTPClient::workThreadFun()
{
	...
	{
		pCommand = NULL;
		if (!HttpExecuter::GetSingleton().popCommand(pCommand))
		{
			Sleep(10);
			continue;
		}
		...
		{
		case enGet:
			get(pCommand->req, pCommand->res, pCommand->result);
			break;
		case enPost:
			post(pCommand->req, pCommand->res, pCommand->data, pCommand->result);
			break;
		default:
			break;
		}
		...
		{
			HttpExecuter::GetSingleton().pushResult(pCommand);
		}
	}
}