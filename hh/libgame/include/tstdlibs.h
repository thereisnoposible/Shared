...
namespace
{
	...
	tstring wstr_to_tstr(const std::wstring& arg)
	{
		...
		#ifdef _WIN32
		size_t size;
		wcstombs_s(&size, const_cast<char*>(res.data()) ,res.length(), arg.c_str(), arg.length());
		#else
        res = Helper::UnicodeToGb2312(arg);
		#endif // _WIN32
        return res;
	}
	...
	std::wstring tstr_to_wstr(const tstring& arg)
	{
		...
		#ifdef _WIN32
		size_t size;
        mbstowcs_s(&size, const_cast<wchar_t*>(res.data()), res.length(), arg.c_str(), arg.length());
        #else
        res = Helper::Gb2312ToUnicode(arg);
        #endif // _WIN32
        return res;
	}
